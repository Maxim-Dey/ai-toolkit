#!/usr/bin/env bash
# Собирает переносимое окружение ai-toolkit и выкладывает архив в out_build.
set -euo pipefail
...
PACK_IMAGE="${PACK_IMAGE:-ai-toolkit-pack}"
...
cid="$(docker run -d "$PACK_IMAGE" bash -c "
    tar -I 'pigz -p \$(nproc)' -cf /tmp/${PACK_NAME}.tar.gz -C /workspace ai-toolkit &&
